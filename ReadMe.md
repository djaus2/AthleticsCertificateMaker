...
## Blog posts
- [davidjones.sportronics.com.au-certs/](https://davidjones.sportronics.com.au/search.html?query=Athletics+AND+Certificate)

# Background
- ***Done:*** Moving all text from the background image so it is generated by the Generate powershell scripts. *This will make the re-use of the certificate generator im other contexts simpler.*
  - **Title,SubTitle,Host and Date** are now all specified in the generate scripts rather than embedded in the background image.
  - There is a separate file, `CertificateText.ps1`, that is included by the scripts that specifies the metadata for these including text, fonts and X,Y locations on the page.
